const stations = [
  ...
  {name:"The Yacht",display:"The Yacht",group:"music",color:"#f0ad2d",desc:"Smooth. Soft. Sailing.",quality:"128",stream:"https://listen.181fm.com/181-yachtrock_128k.mp3"},
  ...
  {name:"Soul",display:"Soul",group:"music",color:"#a66cff",desc:"Classic soul. Timeless groove.",quality:"128",stream:"https://listen.181fm.com/181-soul_128k.mp3"},
...
async function fetchRelay(station,token){
  try{
    ...
    updateMetadata(data.artist || "Super 70s", data.title || "Live Radio");
  }catch(error){
    if(token !== metadataToken) return;
    updateMetadata("Super 70s","Live Radio");
...
function startMetadata(station){
  ...
  if(station.relayId){
    updateMetadata("Checking now playing…","Looking for the groove…");
