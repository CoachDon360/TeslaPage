...
const stations = [
  ...
  {name:"Insight for Living",group:"podcasts",color:"#6bd8ff",desc:"Biblical teaching with Chuck Swindoll.",quality:"PODCAST",stream:"",podcastId:"89603501"},
  {name:"Southeast Christian",group:"podcasts",color:"#70d979",desc:"Messages from Southeast Christian Church.",quality:"PODCAST",stream:"",podcastId:"287341044"},
  {name:"Brian Buffini",group:"podcasts",color:"#ff9b52",desc:"Business, motivation, and real estate.",quality:"PODCAST",stream:"",podcastId:"1089027054"}
];
...
async function fetchPodcastViaApple(station){
  ...
  if(!episodes.length) throw new Error(`No playable episodes returned for ${station.name}`);
...
async function loadPodcast(station,autoplay=false){
  ...
  try{
    ...
    document.getElementById("displayDescription").textContent =
      `${station.name} episode list is temporarily unavailable.`;
    hidePodcastEpisodes();
